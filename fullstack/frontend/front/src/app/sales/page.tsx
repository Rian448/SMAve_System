...
type SalesTab = 'all' | 'premade-sales' | 'pickup-queue';
...
export default function SalesPage() {
  ...
  const getOrderDescription = (order: UnifiedOrder) => {
    if (order.orderType === 'job') return (order as JobOrder).description || 'N/A';
    if (order.orderType === 'product') return 'Premade product purchase';
  ...
  const filteredOrders = orders.filter(order => {
    ...
    if (searchTerm) {
      ...
      return order.customerName.toLowerCase().includes(s) ||
        ...
        (order.vehicleInfo ? `${order.vehicleInfo.year} ${order.vehicleInfo.make} ${order.vehicleInfo.model}`.toLowerCase().includes(s) : false);
    }
    return true;
  });
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-zinc-50 dark:bg-zinc-800/50">
                      <tr>
                        {['Order #', 'Type', 'Customer', 'Vehicle', 'Service', 'Branch', 'Amount', 'Date', 'Status', ''].map(h => (
                          <th key={h} className={`px-6 py-3 text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider ${h === '' ? 'text-right' : 'text-left'}`}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                      {filteredOrders.map(order => (
                        <tr key={`${order.orderType}-${order.id}`} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${order.orderType === 'job' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : order.orderType === 'customer' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' : 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400'}`}>
                          ...
                          <td className="px-6 py-4 text-sm text-zinc-600 dark:text-zinc-300 max-w-xs truncate">{getOrderDescription(order)}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-300">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-300">{formatDate(order.createdAt)}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="space-y-4">
            ...
            {tabLoading ? (
              ...
              <div className="p-10 text-center bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800">
                ...
                <p className="text-zinc-500 dark:text-zinc-400">No premade sales found for this branch.</p>
              </div>
            ) : (
              <div className="space-y-4">
