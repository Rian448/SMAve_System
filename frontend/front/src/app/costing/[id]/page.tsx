...
import { api, JobCosting } from '@/lib/api';
...
export default function CostingDetailPage() {
  ...
  useEffect(() => {
    if (params.id) {
      api.getJobCosting(params.id as string)
        .then(response => {
  ...
  const totalCost = costing.material_cost + costing.labor_cost + costing.overhead_cost;
  const profit = costing.total_price - totalCost;
  const profitMargin = costing.total_price > 0 ? (profit / costing.total_price) * 100 : 0;
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">
              Job Costing #{costing.job_order_id}
            </h1>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-zinc-900 dark:text-white mt-1">
                  ₱{costing.total_price.toLocaleString()}
                </p>
              </div>
            ...
            <div className="mb-6">
              <div className="h-8 flex rounded-lg overflow-hidden">
                <div 
                  ...
                  style={{ width: `${(costing.material_cost / totalCost) * 100}%` }}
                >
                  {((costing.material_cost / totalCost) * 100).toFixed(0)}%
                </div>
                <div 
                  className="bg-green-500 flex items-center justify-center text-xs text-white font-medium"
                  style={{ width: `${(costing.labor_cost / totalCost) * 100}%` }}
                >
                  {((costing.labor_cost / totalCost) * 100).toFixed(0)}%
                </div>
                <div 
                  className="bg-amber-500 flex items-center justify-center text-xs text-white font-medium"
                  style={{ width: `${(costing.overhead_cost / totalCost) * 100}%` }}
                >
                  {((costing.overhead_cost / totalCost) * 100).toFixed(0)}%
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-zinc-900 dark:text-white">₱{costing.material_cost.toLocaleString()}</p>
                  <p className="text-xs text-zinc-500">{((costing.material_cost / totalCost) * 100).toFixed(1)}%</p>
                </div>
              </div>
              <div className="flex items-center justify-between p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-zinc-900 dark:text-white">₱{costing.labor_cost.toLocaleString()}</p>
                  <p className="text-xs text-zinc-500">{((costing.labor_cost / totalCost) * 100).toFixed(1)}%</p>
                </div>
              </div>
              <div className="flex items-center justify-between p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-zinc-900 dark:text-white">₱{costing.overhead_cost.toLocaleString()}</p>
                  <p className="text-xs text-zinc-500">{((costing.overhead_cost / totalCost) * 100).toFixed(1)}%</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800">
                ...
                <span className="text-sm font-medium text-zinc-900 dark:text-white">₱{costing.total_price.toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800">
              ...
              <div className="flex items-center justify-between py-3">
                ...
                <span className="text-sm font-medium text-zinc-900 dark:text-white">
                  ₱{(totalCost / costing.total_price).toFixed(2)}
                </span>
              </div>
            </div>
        ...
        {/* Material Details */}
        {costing.materials && costing.materials.length > 0 && (
              ...
              <table className="w-full">
                <thead className="bg-zinc-50 dark:bg-zinc-800/50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase">Unit Cost</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase">Total</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                  {costing.materials.map((material, index) => (
                    <tr key={index}>
                      <td className="px-4 py-3 text-sm text-zinc-900 dark:text-white">{material.name}</td>
                      <td className="px-4 py-3 text-sm text-center text-zinc-600 dark:text-zinc-400">{material.quantity} {material.unit}</td>
                      <td className="px-4 py-3 text-sm text-right text-zinc-600 dark:text-zinc-400">₱{material.unit_cost.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm text-right font-medium text-zinc-900 dark:text-white">₱{(material.quantity * material.unit_cost).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <button 
            onClick={() => router.push(`/sales/${costing.job_order_id}`)}
