...
export default function DeliveryPage() {
  ...
  const fetchDeliveries = async () => {
    try {
      const data = await api.deliveries.getAll(filter !== 'all' ? { status: filter } : undefined);
      setDeliveries(data);
    } catch (error) {
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-zinc-900 dark:text-white mt-1">{delivery.customerName}</p>
                        <p className="text-sm text-zinc-500 dark:text-zinc-400">{delivery.address}</p>
                        <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">
                          Job Order: {delivery.jobOrderNumber} • Scheduled: {formatDate(delivery.scheduledDate)}
                        </p>
                      </div>
                    </div>
